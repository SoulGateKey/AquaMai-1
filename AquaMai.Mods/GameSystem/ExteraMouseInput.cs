...
namespace AquaMai.Mods.GameSystem
{
    ...
    [ConfigSection(
        en: "Enable radius for DebugMouse input. Enabling this option allows for a more realistic touchscreen experience.",
        zh: "修复在1POnly的MosuseInput.并且让DebugMouse的输入有半径.\n启用此项可以更真实的触摸屏游玩.暂时只支持1P Only.\n如果想用触摸屏/串流玩的话请参考:\n" +
        "Windows 设置 - 蓝牙和其他设备 - 触控 - 三指和四指的触摸手势 - 关闭\n辅助功能 - 鼠标指针与触控 - 触控指示器 - 关闭\n控制面板 - 硬件和声音 - 笔和触控 - 关闭\"触摸回应\"中的\"触摸屏幕时显示直观回应\"\n控制面板 - 硬件和声音 - 笔和触控 - 关闭\"触摸操作\"中的\"将长按用作右键单击\"\n" +
        "把配置文件里的radius调整到和手指一样大小.然后关闭显示" +
        "把A判定延迟调到可以玩的地步就好了(建议关闭判定音) 例如Moonlight的详情显示平均延迟为2.8.那么A判定应该+2.8(超过游戏最大设置的话可以通过延迟修改Mod设置" +
        "简单的iPad串流方案=>Sunshine+Moonlight-ZWM+一个Wifi6的路由器同局域网")]
    public class ExteraMouseInput
    {
        [ConfigEntry(
    en: "Touch Radius.",
    zh: "触摸区域大小")]
        ...
        [ConfigEntry(
    en: "DisplayTouchPoint(Only Touch Input).",
    zh: "显示触摸点(仅限触屏输入)")]
        public static bool displayArea = true;
            ...
            while (true)
            {
                for (int i = 0; i < circleGraphics.Count; i++)
                {
                    if (Input.touchCount > i)
                    {
                        ...
                        // 缩放半径
                        circleGraphics[i].radius = ExteraMouseInput.radius;
